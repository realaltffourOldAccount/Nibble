...
namespace GEngine {
    namespace Text {
        void TextRenderer::Init(void) {
            ...
            t_Shaders = LoadShader(g_config->_glsl_version, "dt", "vs/text.glsl", "fs/text.glsl");
#elif defined(__ANDROID__)
            t_Shaders = LoadShader(g_config->_glsl_version, "es", "vs/text.glsl", "fs/text.glsl");
#endif
            ...
            if (g_config->_openglvMajor >= 3) 
                attrib_pos._index = SHADER_ATTRIB_POS;
            else attrib_pos._index = glGetAttribLocation(t_Shaders->getProgId(), "aPos");
            attrib_pos._size = 4;
        ...
        void TextRenderer::LoadCharacters(std::string fontname) {
            ...
            for (GLubyte c = 32; c < 127; c++) {  // 127
                ...
                if (FT_Load_Char(face, c, FT_LOAD_RENDER)) {
                    Log::warn(
                            "Could not load glyph for '" + std::to_string(c) + "' character.",
                            Log::GenLogID(__LINE__, __FILE__, "TextRenderer", __func__));
                    continue;
                }
                // Generate texture
            ...
            t_Characters.insert(std::pair<std::string, CHARS_LIST>(fontname, characters));
        }
        ...
        void TextRenderer::RenderText(std::string text, std::string fontname,
            ...
            GLCall(glUniform3f(glGetUniformLocation(t_Shaders->getProgId(), "textColor"),
